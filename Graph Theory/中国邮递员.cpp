...
void extend_edge(int add_num) {
    ...
    for (int i = 1; i < n; i++) {
        if ((point[i] & 1) == 1) {
            for (int j = i+1; j <= n; j++) {
                if ((point[j] & 1) == 1 && map[i][j].dis < INF) {
...
int main()
{
    ...
    while (scanf("%d %d", &n, &m) != EOF) {
        ...
        if (odd_point > 2)  /// 奇点个数大于两个的时候，用floyd算法更新任意两点之间最短路径，并且添加边
        {
            Floyd();
        ...
        top = 0;
        if (odd_point == 2) /// 奇点个数为2的时候，从一个奇点出发到另一个奇点去，但不能构成环路，直接退出程序
        {
            for (int i = 1; i <= n; i++)
            {
                if ((point[i] & 0x1) == 1)
